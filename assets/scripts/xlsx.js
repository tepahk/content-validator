...
function writetag(f,g) {
	return '<' + f + ( typeof g === 'string' && g.match(wtregex)?' xml:space="preserve"' : "") + '>' + g + '</' + f + '>';
}

function wxt_helper(h) { return keys(h).map(function(k) { return " " + k + '="' + h[k] + '"';}).join(""); }
function writextag(f,g,h) { return '<' + f + (isval(h) ? wxt_helper(h) : "") + (isval(g) ? (typeof g === 'string' && g.match(wtregex)?' xml:space="preserve"' : "") + '>' + g + '</' + f : "/") + '>';}
